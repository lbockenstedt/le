...
`install_le.sh`: `--hub`, `--id`/`--name`, `--secret`, `--hub-secret`, `--all-prereqs` (no-op). Default `SPOKE_ID=le-<hostname>`, `HUB_URL=auto` (auto-discovery via DNS `lm-hub.<suffix>` then mDNS; pass `--hub wss://<host>:443` to pin a remote hub). **No** `--tls-verify`/`--tls-ca-cert` (not wired in this installer).

## Key commands / handlers (`LESpoke.handle_command`, `src/le_spoke.py`)

`LE_GET_STATUS` (version, `certbot_present`, certs managed), `LE_LIST_CERTS`, `LE_GET_CERT` (returns `fullchain`/`privkey`/`chain`/`material_hash`/`not_after`; `privkey` masked in logs but returned for distribution), `LE_ISSUE_CERT` (`domain`, `email`, `challenge` http|dns|tls-alpn, `webroot`, `dns_provider`, `dns_creds`/`dns_creds_ini`, `staging`, `key_type`, `targets[]`), `LE_SET_CLIENTAUTH` (`domain`, `client_auth`), `LE_ACME_INFO`, `LE_RENEW_CERT` (one domain or all), `LE_SYNC_VAULT_DNS` (vault-synced DNS hook creds), `LE_REVOKE_CERT` (`domain`, `delete`), `LE_ADD_TARGET` (`domain`, `target.module_type`, `target.identifier`), `LE_REMOVE_TARGET` (`domain`, `idx` — positional target index), `LE_MARK_DISTRIBUTED` (`domain`, `module_type`, `identifier`, `hash`, `status`, `message`), `LE_DEPLOY_TO_AGENT` (`agent_id`, `domain`, `helper_path`), `LE_LIST_DNS_CREDS` (`tenant_id`), `LE_SET_DNS_CRED` (`tenant_id`, `name`, `provider`, `fields`), `LE_DELETE_DNS_CRED` (`tenant_id`, `name`), `LE_SET_HE_LOGIN` (`he_username`, `he_password`), `LE_GET_HE_LOGIN`, `UPDATE_CONFIG` (honors `renew_interval`), `GET_VERSION`, `SET_LOG_LEVEL`.
...
- TLS-ALPN-01: `--preferred-challenges tls-alpn-01`. certbot ships no built-in authenticator for this challenge — it requires a host-installed TLS-ALPN-01 plugin (not auto-installed, unlike the DNS-01 plugins above, since there's no standard apt package). Absent one, certbot fails with its own clear "no authenticator" error.
...
- **Real certbot, not stubs** — `src/acme.py` shells out to the real `certbot` binary for issue/renew/revoke; the ledger is the parallel index, not a placeholder.
